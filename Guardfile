...
guard :shell do
  ...
  watch( /(.+?)\/(.*)\.yml$/ ) do |m|
    ...
    ansible_guard_playbook_tmpdir = File.join( ansible_guard_tmpdir, 'playbooks', role_name )

    FileUtils.mkdir_p File.join( ansible_guard_tmpdir, 'roles' )
    FileUtils.mkdir_p File.join( ansible_guard_playbook_tmpdir )
    ...
    # puts "creating symlink: #{File.join( ansible_guard_tmpdir, 'roles', File.basename(Dir.pwd) )} -> #{Dir.pwd} "
    # puts "creating symlink: #{File.join( ansible_guard_playbook_tmpdir, 'roles' )} -> #{File.join( ansible_guard_tmpdir, 'roles')} "
    
    File.symlink( Dir.pwd, File.join( ansible_guard_tmpdir, 'roles', File.basename(Dir.pwd) ) )
    File.symlink( File.join( ansible_guard_tmpdir, 'roles'), File.join( ansible_guard_playbook_tmpdir, 'roles' ) )

    begin
      Tempfile.create("#{role_name}.yml", ansible_guard_playbook_tmpdir) do |file|
        
        File.open(file, 'w') do |test_playbook|
          file.write <<-EOF.gsub(/^ {12}/, '')
            # Temporary playbook file to include the role to syntax-check
            ---
            - name: wrapper playbook for testing "#{role_name}"
              hosts: localhost
              roles:
                - #{role_name}
          EOF
        ...
        Dir.chdir ansible_guard_tmpdir do
          ...
          if system("ansible-playbook -i #{File.join( ansible_guard_tmpdir, 'inventory' )} --syntax-check #{file.path}")
            n "Ansible role syntax-check passed!", role_name, :success
          else
            n "Ansible role syntax-check FAILED!", role_name, :failed
          end
        end
      end
    ensure
